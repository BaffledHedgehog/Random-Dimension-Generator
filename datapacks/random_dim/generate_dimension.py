...
Каждый запуск добавляет в датапак +1 новое измерение со случайными:
  ...
  * шумами генерации: собственные случайные спектры - пять форм
    ...
    держат свою нормализацию ? (находимость биомов)
  ...
  * подземными биомами: в мирах выше 256 с 6+ биомами 2-4 биома становятся
    ...
    уровнями богатства x0.3/x1/x2.5/x5 и высотами по долям высоты мира
  * каменным семейством: 3-7 доп. блоков с тирами частоты и РАЗНЫМИ
    ...
    15-64xcount 1-6), 1-2 глубинные полосы (как deepslate), 1-3
...
# ДРЕВЕСИНА - вес x0.25 от прежнего (16 -> 4): брёвна/обтёсанные/
...
# x0.25): «рельеф из брёвен» теперь редкость, камнеподобные (вес 20)
# - наоборот, чаще
BLOCK_TIERS = [
...
# Подземная climate-depth зона; её Y-диапазон задаёт _prepare_climate.
# Дно с бедроком исключено, верх ниже номинального уровня поверхности.
# Полосы не пересекаются; в зазоре побеждает БЛИЖАЙШАЯ пещерная полоса
# (не поверхностный биом). Одна пара констант для генерации и тестов.
CAVE_BAND_TOP = 0.2
CAVE_BAND_BOTTOM = 1.45
# Default-world target, not a per-seed promise: retain 5% unusual sources.
CAVE_WORLD_TARGET = 0.95
CAVE_MIN_BIOMES = 4  # two surface + at least two distinct cave strata
...
# Квантильные центры кластеров рельефа: ???((2i+1)/(2n)) - медианы n
# равных долей N(0, ?_cont) (n=3 -> квантили 1/6, 3/6, 5/6 и т.д.).
...
# классно выглядел»). 34 архетипа x рандомизация внутри окон = сотни
...
#   p_frac     - доля доступного бюджета возмущений P (P = p_frac x
...
#   void_cut   - (опц.) окно порога среза островов (доля ? поля):
...
OPEN_PRESETS = [
    ...
    dict(name="pure_chaos", w=2.0, cat="interesting",
         ...
         cap_hi=(0.32, 0.45),
         flavors={"normal": 3, "spire": 3, "ridge": 3, "wave": 3, "strata": 2},
         env="rising", ops=(3, 3, 3), mix_p=0.85, spline_p=0.6,
         void_style="shards", void_cut=(0.30, 0.90)),
    dict(name="cosmic_abyss", w=1.5, cat="interesting",
         desc="Космическая бездна: гравитационные провалы, парящие кольца",
         character="craters",
         b=(0.50, 0.80), p_frac=(0.85, 1.00), layers=(3, 5),
         cap_hi=(0.30, 0.45),
         flavors={"wave": 5, "spire": 4, "ridge": 3},
         env="peak", ops=(4, 2, 4), mix_p=0.50, spline_p=0.8),
    dict(name="floating_archipelago", w=1.5, cat="interesting",
         desc="Парящий архипелаг: многоярусные летающие острова",
         character="strata",
         b=(0.55, 0.85), p_frac=(0.80, 1.00), layers=(2, 4),
         cap_hi=(0.28, 0.45),
         flavors={"strata": 6, "spire": 4, "wave": 2},
         env="decay", ops=(5, 2, 4), mix_p=0.45, spline_p=0.8,
         void_style="rafts", void_cut=(0.40, 0.85)),
    dict(name="crystalline_spires", w=1.5, cat="interesting",
         desc="Кристальные шпили: гигантские гранёные колонны",
         character="spiky",
         b=(0.55, 0.80), p_frac=(0.85, 1.00), layers=(3, 5),
         cap_hi=(0.32, 0.45),
         flavors={"spire": 8, "strata": 3, "ridge": 2},
         env="rising", ops=(5, 1, 4), mix_p=0.40, spline_p=0.7),
    dict(name="volcanic_caldera", w=1.5, cat="interesting",
         desc="Вулканическая кальдера: ступенчатые разломы и кратеры",
         character="craters",
         b=(0.60, 0.90), p_frac=(0.80, 1.00), layers=(2, 4),
         cap_hi=(0.30, 0.44),
         flavors={"ridge": 5, "wave": 4, "normal": 2},
         env="decay", ops=(4, 2, 4), mix_p=0.35, spline_p=0.9),
    dict(name="fractal_labyrinth", w=1.5, cat="interesting",
         desc="Фрактальный лабиринт: извилистые каньоны и переплетённые хребты",
         character="canyon",
         b=(0.55, 0.85), p_frac=(0.85, 1.00), layers=(3, 5),
         cap_hi=(0.30, 0.45),
         flavors={"ridge": 7, "normal": 3, "wave": 2},
         env="peak", ops=(3, 2, 5), mix_p=0.50, spline_p=0.8),
    dict(name="gravity_inversion", w=1.5, cat="interesting",
         desc="Гравитационная инверсия: нависающие скальные своды и арки",
         character="chaotic",
         b=(0.50, 0.78), p_frac=(0.90, 1.00), layers=(3, 5),
         cap_hi=(0.32, 0.45),
         flavors={"wave": 5, "spire": 4, "strata": 3},
         env="rising", ops=(3, 3, 4), mix_p=0.60, spline_p=0.7),
    dict(name="alien_badlands", w=1.0, cat="interesting",
         desc="Инопланетные пустоши: ступенчатые уступы и иглы",
         character="terraced", steps=(4, 8),
         b=(0.60, 0.95), p_frac=(0.75, 1.00), layers=(2, 4),
         cap_hi=(0.28, 0.42),
         flavors={"strata": 6, "spire": 3, "normal": 2},
         env="decay", ops=(6, 1, 3), mix_p=0.35, spline_p=0.9),
    dict(name="subterranean_breach", w=1.0, cat="interesting",
         desc="Подземный прорыв: гигантские открытые расщелины",
         character="canyon",
         b=(0.60, 0.90), p_frac=(0.85, 1.00), layers=(2, 5),
         cap_hi=(0.30, 0.45),
         flavors={"ridge": 6, "normal": 3, "spire": 2},
         env="decay", ops=(4, 1, 5), mix_p=0.40, spline_p=0.9),
    dict(name="shattered_glacier", w=1.0, cat="interesting",
         desc="Расколотый ледник: ледяные пики и бездонные трещины",
         character="spiky",
         b=(0.55, 0.85), p_frac=(0.80, 1.00), layers=(3, 5),
         cap_hi=(0.32, 0.45),
         flavors={"spire": 7, "ridge": 3, "normal": 2},
         env="peak", ops=(5, 1, 4), mix_p=0.30, spline_p=0.9),
    dict(name="whispering_crags", w=1.0, cat="interesting",
         desc="Шепчущие утесы: ветровые скалы и каменные гребни",
         character="mountainous",
         b=(0.60, 0.85), p_frac=(0.85, 1.00), layers=(3, 5),
         cap_hi=(0.30, 0.44),
         flavors={"ridge": 6, "spire": 3, "wave": 2},
         env="decay", ops=(4, 1, 5), mix_p=0.35, spline_p=1.0),
    dict(name="prismatic_terraces", w=1.0, cat="interesting",
         desc="Призматические террасы: ступенчатые чаши и пороги",
         character="terraced", steps=(5, 9),
         b=(0.65, 0.95), p_frac=(0.70, 0.95), layers=(2, 4),
         cap_hi=(0.26, 0.40),
         flavors={"strata": 5, "wave": 4, "normal": 2},
         env="flat", ops=(9, 1, 1), mix_p=0.30, spline_p=0.9),
    dict(name="titan_spine", w=1.0, cat="interesting",
         desc="Хребет Титана: циклопическая изогнутая горная цепь",
         character="mountainous",
         b=(0.60, 0.85), p_frac=(0.85, 1.00), layers=(3, 5),
         cap_hi=(0.32, 0.45),
         flavors={"ridge": 8, "spire": 2, "normal": 1},
         env="peak", ops=(3, 1, 6), mix_p=0.25, spline_p=1.0),
    dict(name="hollow_plateau", w=1.0, cat="interesting",
         desc="Полое плато: возвышенности с внутренними гротами и арками",
         character="terraced", steps=(3, 6),
         b=(0.65, 0.95), p_frac=(0.80, 1.00), layers=(3, 5),
         cap_hi=(0.30, 0.44),
         flavors={"strata": 5, "ridge": 3, "normal": 3},
         env="decay", ops=(6, 1, 3), mix_p=0.40, spline_p=0.8),
    dict(name="void_needles", w=1.0, cat="interesting",
         desc="Иглы пустоты: тончайшие каменные столбы до облаков",
         character="spiky",
         b=(0.60, 0.85), p_frac=(0.85, 1.00), layers=(3, 5),
         cap_hi=(0.35, 0.45),
         flavors={"spire": 9, "strata": 1},
         env="rising", ops=(9, 1, 1), mix_p=0.15, spline_p=0.8,
         void_style="needles"),
    dict(name="basalt_columns", w=1.0, cat="interesting",
         desc="Базальтовые колонны: шестигранные ступени и башни",
         character="terraced", steps=(4, 7),
         b=(0.65, 0.95), p_frac=(0.80, 1.00), layers=(2, 4),
         cap_hi=(0.30, 0.45),
         flavors={"strata": 6, "spire": 3, "normal": 2},
         env="decay", ops=(6, 1, 3), mix_p=0.30, spline_p=0.9),
    dict(name="dune_sea_extreme", w=1.0, cat="interesting",
         desc="Великое песчаное море: грандиозные барханы с отвесными гребнями",
         character="dunes",
         b=(0.55, 0.85), p_frac=(0.75, 1.00), layers=(2, 3),
         cap_hi=(0.25, 0.38),
         flavors={"wave": 7, "ridge": 3, "normal": 1},
         env="peak", ops=(9, 1, 1), mix_p=0.25, spline_p=0.9),
    dict(name="serpentine_gorge", w=1.0, cat="interesting",
         desc="Змеиное ущелье: головокружительный извилистый каньон",
         character="canyon",
         b=(0.60, 0.90), p_frac=(0.80, 1.00), layers=(2, 4),
         cap_hi=(0.28, 0.44),
         flavors={"ridge": 6, "normal": 3, "wave": 2},
         env="decay", ops=(4, 1, 5), mix_p=0.35, spline_p=1.0),
    dict(name="meteor_basin", w=1.0, cat="interesting",
         desc="Метеоритная котловина: воронка с центральным пиком",
         character="craters",
         b=(0.65, 0.95), p_frac=(0.75, 1.00), layers=(2, 4),
         cap_hi=(0.28, 0.42),
         flavors={"wave": 5, "spire": 3, "normal": 3},
         env="decay", ops=(6, 1, 3), mix_p=0.30, spline_p=0.9),
    dict(name="cyclopean_blocks", w=1.0, cat="interesting",
         desc="Циклопические блоки: гигантские мегалитические уступы",
         character="terraced", steps=(3, 5),
         b=(0.70, 1.00), p_frac=(0.75, 1.00), layers=(2, 4),
         cap_hi=(0.30, 0.45),
         flavors={"strata": 7, "spire": 2, "normal": 2},
         env="flat", ops=(6, 1, 3), mix_p=0.25, spline_p=0.8),
    ...
    dict(name="steppe_plateaus", w=3.0, cat="moderate",
         desc="Степные плато: пологие возвышенности",
         character="terraced", steps=(3, 5),
         b=(0.70, 1.05), p_frac=(0.50, 0.75), layers=(1, 3),
         cap_hi=(0.18, 0.28),
         flavors={"strata": 5, "normal": 5},
         env="decay", ops=(9, 1, 1), mix_p=0.20, spline_p=1.0),
    dict(name="hilly_meadows", w=3.0, cat="moderate",
         desc="Холмистые луга: мягкие волнообразные холмы",
         character="rolling",
         b=(0.70, 1.05), p_frac=(0.50, 0.75), layers=(2, 3),
         cap_hi=(0.18, 0.28),
         flavors={"wave": 6, "normal": 4},
         env="decay", ops=(9, 1, 1), mix_p=0.25, spline_p=1.0),
    dict(name="coastal_swells", w=2.5, cat="moderate",
         desc="Прибрежные увалы: широкие покатые гряды",
         character="rolling",
         b=(0.75, 1.10), p_frac=(0.45, 0.70), layers=(1, 2),
         cap_hi=(0.16, 0.25),
         flavors={"wave": 7, "normal": 3},
         env="peak", ops=(9, 1, 1), mix_p=0.15, spline_p=1.0),
    ...
    dict(name="endless_savanna", w=2.0, cat="boring",
         desc="Бескрайняя ровная саванна",
         character="flat",
         b=(0.85, 1.15), p_frac=(0.30, 0.50), layers=(1, 2),
         cap_hi=(0.12, 0.18),
         flavors={"normal": 7, "wave": 3},
         env="decay", ops=(9, 1, 1), mix_p=0.10, spline_p=0.9),
...
class DimensionGenerator:
    ...
    def new_noise(self, pref=None):
        ...
        Возвращает ID. Формы огибающей (спектра), выбор случайно:
          decay   - классическая убывающая (каждая следующая x0.3-0.7):
        ...
        climate-каналы его НЕ используют: у тех своя нормализация ?
    ...
    def rand_final_density(self):
        ...
        * КОМПОЗИЦИИ РАЗНООБРАЗИЯ: пресет x характер базы x
          региональный микс (range_choice по медленному 2D-шуму - в
          двух половинах мира РАЗНЫЕ стеки возмущений) x вертикальный
          сплит cavern (свои возмущения под поясом и над) x сплайн
        ...
        def base_form(zlo, zhi, flip=False, k=1.0):
            ...
            if character == "chaotic":
                turb = cl(mulc(rnd_f(rng, 0.8, 1.6), nz(rnd_f(rng, 0.015, 0.05), rnd_f(rng, 0.02, 0.08), env="rising")), -0.45, 0.45)
                ramp = grad(zlo, zhi, B, -k * B) if not flip else grad(zlo, zhi, -k * B, B)
                return add(ramp, turb)
        ...
        # --- острова void: НОРМАЛИЗОВАННЫЕ шумы с известной ? ---
        def island_field():
            """Поле островов void-мира: 1-2 3D-шума с ЗАРАНЕЕ известной
            ? (амплитуды нормированы как у climate-каналов - случайный
            спектр без нормализации то пуст (?~0.05), то насыщен
            (?~1.5): половина void-миров была без единого острова).
        ...
        if self.world_shape == "void":
            ...
            core = {"type": "minecraft:max", "argument1": core,
                    "argument2": roof_cap()}
        # Dedicated noise cavities, independent of carver start-biome sampling.
        # Add a local rock shell as well as air: even island worlds have enclosed
        # caves rather than cave-biome labels floating in otherwise empty space.
        if self.biome_underground:
            core = self._with_cave_geometry(core)
    ...
    def _cave_y_bounds(self, band):
        """Inverse of the actual climate depth ramp (absolute inclusive Y)."""
        depth = self.climate_ch["depth"]
        span = depth["to_y"] - depth["from_y"]
        def at(value):
            return depth["from_y"] + span * (1.5 - value) / 3.0
        return math.ceil(at(band[1])), math.floor(at(band[0]))

    def _with_cave_geometry(self, core):
        """Noise-shaped, dry, bounded cavities with real floors and ceilings.

        Diverse multi-noise generator: each archetype gets its own distinct
        octave and frequency profile, combining primary chamber noise with
        secondary 3D noodle/corridor noise. In ~40% of open worlds, the
        ...
        self.cave_geometry = {}
        strata_info = []
        for bid in sorted(self.biome_underground):
        ...
        surface_breach = getattr(self, "surface_breach", False)
        highest_bid = max(strata_info, key=lambda s: s[3])[0] if strata_info else None

        for bid, lo, hi, center, half, arch, fo, amps, scale, width, ys, n_scale in strata_info:
            pri_noise = {"type": "minecraft:noise",
                         "noise": self._add_octaved_noise(fo, amps),
                         "xz_scale": scale, "y_scale": ys}
            sec_noise = {"type": "minecraft:noise",
                         "noise": self._add_octaved_noise(-2, [1.0, 0.5]),
                         "xz_scale": n_scale, "y_scale": round(ys * 1.5, 4)}
            # Multi-noise combination: primary chamber + secondary 3D noodle tunnel
            ...
            outer = op("max", op("add", outer_y, -1.9),
                       op("add", lateral, -(width + 0.18)))
            inner = op("max", op("add", vertical, -0.72),
                       op("add", lateral, -width))
            core = self.new_df_file(core)
            ...
            core_normal = {"type": "minecraft:range_choice",
                           ...
                           "min_inclusive": 0.0001, "max_exclusive": 0.9999,
                           "when_in_range": local, "when_out_of_range": core}

            if surface_breach and bid == highest_bid and self.world_shape == "open":
                ...
                vert_breach = {"type": "minecraft:abs", "argument": {
                    "type": "minecraft:y_clamped_gradient",
                    "from_y": center - int(half), "to_y": breach_top,
                    "from_value": -1.0, "to_value": 1.0}}
                inner_breach = op("max", op("add", vert_breach, -0.78), op("add", lateral, -width))
                local_breach = op("min", core, inner_breach)
                local_breach = {"type": "minecraft:clamp", "input": local_breach, "min": -4.0, "max": 4.0}
                core_breached = {"type": "minecraft:range_choice",
                                 "input": {"type": "minecraft:y_clamped_gradient",
                                           "from_y": lo, "to_y": breach_top,
                                           "from_value": 0.0, "to_value": 1.0},
                                 "min_inclusive": 0.0001, "max_exclusive": 0.9999,
                                 "when_in_range": local_breach, "when_out_of_range": core}
                core = {"type": "minecraft:range_choice",
                        "input": breach_mask,
                        "min_inclusive": 0.0, "max_exclusive": 0.15,
                        "when_in_range": core_breached, "when_out_of_range": core_normal}
            else:
                core = core_normal

            self.cave_geometry[bid] = {"min_y": lo, "max_y": hi,
                                       "center_y": center, "half_height": half,
                                       "xz_scale": scale, "width": width,
                                       "surface_breach": (surface_breach and bid == highest_bid)}
        return core
    ...
    def climate_noise_2d(self):
        ...
        ridges): горизонтальный 2D-шум с ЗАРАНЕЕ ИЗВЕСТНОЙ ? - амплитуды
        нормируем к target 0.35..0.7 (каждая октава ~N(0,(0.25·amp)?),
        итог 0.25·sqrt(?amp?)). ? канала размечает квантильную решётку
        ...
        (юзер: «сами биомы чуть больше по размерам»), а ?-нормализация
    ...
    def continents_noise_2d(self):
        """continents-канал: 2D-шум с ? 0.6..1.1 (крупнее климатических -
    ...
    def _channel_sigma(self, df):
        """? климатического канала по его DF (нужна квантильной решётке
        биом-сорса): шум - 0.25·sqrt(?amp?); всё прочее (константа,
    ...
    def _prepare_climate(self):
        ...
        рельефа и биом-сорса: ? temperature/vegetation нужна квантильной
        решётке биом-сорса, ? continents - квантильным центрам кластеров,
        а сами DF потом просто вставляются в router
        (rand_noise_settings). В cave-mode depth - рампа 1.5->-1.5:
        1.5 сразу над твёрдым капом дна, 0 у 46% высоты (в cavern -
        не выше воздушного пояса). Это ВЫСОТНОЕ распределение, а не
        вычисление локальной поверхности/heightmap; низкие открытые
        обрывы могут показывать подземный биом. Полосы 0.2..1.45
        резервируют место реальным полостям даже при height=128.
        Остальные источники сохраняют старую рампу по всей высоте."""
        self.climate_ch = {
        ...
        if self.cave_mode:
            bottom = self.min_y + self.density_band + 4
            surface = self.min_y + int(self.height * 0.46)
            # For cavern worlds keep every dedicated cave shell below the
            # guaranteed central air belt; retain at least 32 blocks at H128.
            if self.world_shape == "cavern":
                surface = min(surface, self.belt_lo - 2)
            self.climate_ch["depth"] = {
                "type": "minecraft:y_clamped_gradient",
                "from_y": bottom, "to_y": bottom + 2 * (surface - bottom),
                "from_value": 1.5, "to_value": -1.5}
    ...
    def _pick_top_block(self, rng, pool, used_top, recent_fams):
        ...
        Дерево в top само по себе теперь редко (вес x0.25 в
        BLOCK_TIERS) - отдельного запрета не нужно."""
        # Filter the gravity budget BEFORE material-family preference. A fresh
        # family may consist solely of sand/gravel; falling-aware_choice's
        # all-falling fallback must not silently exceed the world budget.
        if len(self._falling_used) >= self._falling_budget:
            pool = [b for b in pool if b[0] not in FALLING_BLOCK_IDS]
    ...
    def rand_surface_rule(self, default_block):
        ...
        if self.biome_underground:
            # Bedrock remains first; general random patches cannot erase the
            # archetype floor/ceiling/body before biome rules get a chance.
            for i, rule in enumerate(seq):
                if "bedrock" not in json.dumps(rule):
                    seq[i] = {"type": "minecraft:condition",
                              "if_true": {"type": "minecraft:not", "invert": {
                                  "type": "minecraft:biome",
                                  "biome_is": sorted(self.biome_underground)}},
                              "then_run": rule}
                    ...
                    layers_seq.append({
                        ...
                        "then_run": {"type": "minecraft:block",
                                     "result_state": block_state(
                                         arch_ceil[bid])}})
                if bid in arch_ceil:
                    layers_seq.insert(0, layers_seq.pop())
    ...
    def _rand_biome_features(self, idx, band, prev_sets):
        """Свои фичи биома (1-6 configured x 1-2 placed, префикс
        ...
        while len(kinds) >= 2:
            ...
            drop = min(over, key=lambda t: (sum(
                1 for v in tmap.values() if v == t), t))
    ...
    def rand_biome(self, biome_feats, biome_carvers, biome_mobs, band=None,
            ...
            if arch:
                for f in arch.get("vanilla", []):
                    if f not in vfeats and rng.random() < 0.7 \
                        ...
                        vfeats.add(f)
        if underground:
            # Monster rooms are decorations, not the cave identity. One at
            # most; no generic lush/dripstone/sculk bonuses in other archetypes.
            rooms = sorted(f for f in vfeats if "monster_room" in f)
            vfeats.difference_update(rooms[1:])
            themed = set((arch or {}).get("vanilla", []))
            vfeats = {f for f in vfeats if f in themed
                      or f in LAKE_CLASS_FEATURES or f.startswith("ore_")
                      or "monster_room" in f or f == "glow_lichen"}
        for f in sorted(vfeats):
            fid = "minecraft:" + f
            features[self._feature_step_of(fid)].append(fid)

        # СВОИ placed-фичи биома - все до единого (1-6 configured x 1-2
        ...
        # СВОЁМ уровне богатства - x0.3/x1/x2.5/x5 (или отсутствует, 12%).
    ...
    def _biome_source_multi_noise(self, ids):
        """Full horizontal tiling + disjoint depth bands.

        Surface cells tile temperature/humidity/continentalness without holes;
        underground strata span those axes. Thus horizontal climate cannot
        outweigh the depth separation. In a gap the NEAREST cave stratum wins,
        not a fictitious return to the surface biome. Offset/erosion/ridges are
        equal for all entries. This is altitude separation, not a heightmap.
        """
        rng = self.rng
        # надземные - по кластерам рельефа (биомы кластера делят
        # пространство temperaturexhumidity, кластеры разведены по
        ...
        active = sorted(groups, key=lambda cid: self.terr_clusters[cid][0]) if corr else sorted(groups)
        cont_ranges = {}
        for i, cid in enumerate(active):
            lo = -1.5 if i == 0 else (self.terr_clusters[active[i-1]][0] + self.terr_clusters[cid][0]) / 2
            hi = 1.5 if i == len(active)-1 else (self.terr_clusters[cid][0] + self.terr_clusters[active[i+1]][0]) / 2
            cont_ranges[cid] = [round(lo, 4), round(hi, 4)] if corr else [-1.5, 1.5]
        inv = statistics.NormalDist().inv_cdf
        biomes = []
        for cid in sorted(groups):
            group = groups[cid]
            k = len(group)
            # решётка axb ячеек (a=ceil(sqrt(k)), b=ceil(k/a)) >= k:
    ...
    def rand_noise_settings(self):
        ...
        if self.biome_underground:
            # Negative density must remain AIR, not a hidden lava/solid sea.
            # Keep occasional liquids/features, but never flood cave strata.
            aquifers = False
            sea_level = min(sea_level, self.climate_ch["depth"]["from_y"] - 1)
            size_v = 1  # four-block cells preserve short H128 chambers
        ...
        # _prepare_climate: это плоские шумы с ЗАРАНЕЕ известной ?, по
    ...
    def generate(self, biome_count=None, prev_dim=None):
        ...
        if self.world_shape == "cavern":
            ...
            # Leave room for two small enclosed cave strata below the air belt,
            # including in 128-block worlds; the central air guarantee remains.
            blo = max(self.min_y + self.density_band + t_band, bc - half,
                      self.min_y + self.density_band + 36)
        ...
        # и 3 кластерах), ячейки равновелики по квантилям ?-каналов
        ...
        self.bs_kind = rng.choices(
            ...
            weights=[int(100 * CAVE_WORLD_TARGET), 2, 1, 1, 1])[0]
        ...
        self.surface_breach = (self.world_shape == "open" and self.terrain_corr and self.height >= 128 and ((_fnv1a(self.name + "_breach") ^ self.height) & 0xFFFF) % 100 < 45)
        self.cave_mode = (self.terrain_corr and self.height >= 128
                          and biome_count >= CAVE_MIN_BIOMES)
        # climate-каналы router'а готовим ЗАРАНЕЕ: ? continents нужна
        # квантильным центрам кластеров ниже, ? temperature/vegetation -
        ...
        # центры кластеров - КВАНТИЛИ N(0, ?_cont): медианы n равных
        ...
        # богатства x0.3/x1/x2.5/x5 с id <name>_oreM_pK. Биомы получают
        ...
        if n_carv:
            self.carvers_cfg = gen_structures.rand_carvers(
                rng, self.ns, self.name, self.min_y, self.max_y - 1,
                count=n_carv)
        # All eligible multi_noise worlds receive distinct cave archetypes.
        # Non-vertical fixed/checkerboard/preset/end sources remain unusual.
        self.biome_underground = set()
        self.biome_band = {}
        ...
        if self.cave_mode:
            ...
            # At least ~16 vertical blocks per stratum, including shell.
            bottom = self.climate_ch["depth"]["from_y"]
            zero_y = (bottom + self.climate_ch["depth"]["to_y"]) / 2
            capacity = max(2, int((zero_y - bottom) * 0.83 / 16))
            n_under = min(n_under, len(CAVE_ARCHETYPES),
                          max(2, biome_count - 2), capacity)
            ...
            # зазоры 0.05, малый джиттер ширины 0.95..1.05. Сильный
            # джиттер давал слишком узкие для шумовой ячейки H128 полосы.
            gap = 0.05
            budget = (CAVE_BAND_BOTTOM - CAVE_BAND_TOP
                      - gap * (n_under - 1))
            wts = [rnd_f(rng, 0.95, 1.05) for _ in range(n_under)]
            wsum = sum(wts)
            edge = CAVE_BAND_TOP
            for j, idx in enumerate(under_idx):
                w = budget * wts[j] / wsum
        ...
        self.biomes = {}
        # Reserve future cave signatures before ANY surface biome is generated;
        # otherwise a later cave can collide with an already accepted surface.
        own_sets = [_arch_feature_types(CAVE_ARCHETYPES[a])
                    for a in sorted(set(arch_of_idx.values()))]
        for i in range(biome_count):
            ...
            if band is not None:
                ...
                bfc, bfp, btags = gen_features.rand_cave_features(
                    ...
                    terrain=arch["stone"][0],
                    floor_blocks=[arch["floor"], arch["stone"]] + arch["sub"],
                    no_gravity=self.world_shape == "void",
                    height_bounds=self._cave_y_bounds(band))
        ...
        if gl is not None and hasattr(gl, "set_enchantment_context"):
            gl.set_enchantment_context(self.ench.get("enchantments", {}),
                                       self.ench.get("function_metadata", {}))
        ...
        # Автоматическая установка точки возрождения игрока при переходе в новое измерение
        spawn_adv_id = f"{self.ns}:enter_{self.name}"
        spawn_fn_name = f"enter_{self.name}"
        self.adv["advancements"][spawn_adv_id] = {
            "criteria": {
                "enter_dim": {
                    "trigger": "minecraft:changed_dimension",
                    "conditions": {"to": dim_id},
                }
            },
            "rewards": {"function": f"{self.ns}:{spawn_fn_name}"},
        }
        self.adv["functions"][spawn_fn_name] = (
            f"execute if dimension {dim_id} run spawnpoint @s ~ ~ ~\n"
            f"advancement revoke @s only {spawn_adv_id}\n"
        )
        ...
        return {
            ...
            # Python-side loot/audit context, deliberately not a registry file.
            "function_metadata": dict(self.ench.get("function_metadata", {})),
...
def _vnoise(x, y, z, s):
    ...
    так, чтобы ? октавы равнялась ванильной 0.25*amp (без калибровки
...
def _sim_noise_sample(spec, x, y, z, s):
    ...
    статистика (? ~ 0.25*sqrt(?amp?)) достаточно близка к ванильному
...
def _self_test_worldgen():
    ...
    - climate-каналы: окна спеки (юзер: биомы чуть крупнее) -
      firstOctave -10..-7, xz 0.10..0.28, y 0.0, ? 0.35..0.7;
      continents: firstOctave -9..-7, ? 0.6..1.1, волна октавы-0
    ...
    try:
        ...
        while (tall < 4 or flat < 3 or voids < 3) and seed < 250:
            ...
            # >= 2xmax(6, 9%·h) ширины (см. belt в generate) -> доля
            ...
            qualified = (gen.height >= 128 and len(gen.biomes) >= CAVE_MIN_BIOMES
                         and gen.bs_kind == "multi_noise")
            if qualified:
                tall += 1
                # 128-255 - 2-3 биома, >= 256 - 3-6 (как в generate)
                lo_n, hi_n = ((2, 6) if gen.height >= 256 else (2, 3))
